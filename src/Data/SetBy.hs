...
lt cmp x y = case cmp x y of { LT -> True; _ -> False }
gt cmp x y = case cmp x y of { GT -> True; _ -> False }
{--------------------------------------------------------------------
  Operators
--------------------------------------------------------------------}
infixl 9 \\ --
...
member x (Set cmp t) = member' cmp x t
member' cmp x t
  = case t of
      Tip -> False
      Bin _ y l r
...
insert x (Set cmp s) = Set cmp (insert' cmp x s) 
insert' cmp x t
  = case t of
      Tip -> singleton' x
      Bin sz y l r
          -> case cmp x y of
               LT -> balance' y (insert' cmp x l) r
               GT -> balance' y l (insert' cmp x r)
               EQ -> Bin sz x l r


-- | /O(log n)/. Delete an element from a set.
delete :: {- Ord a => -} a -> Set a -> Set a
delete x (Set cmp s) = Set cmp $ delete' cmp x s
delete' cmp x t
  = case t of
      Tip -> Tip
      Bin _ y l r
          -> case cmp x y of
               LT -> balance' y (delete' cmp x l) r
               GT -> balance' y l (delete' cmp x r)
               EQ -> glue' l r
...
isSubsetOfX (Set cmp1 l) (Set cmp2 r) = isSubsetOfX' cmp1 l r
isSubsetOfX' cmp Tip _ = True
isSubsetOfX' cmp _ Tip = False
isSubsetOfX' cmp (Bin _ x l r) t
  = found && isSubsetOfX' cmp l lt && isSubsetOfX' cmp r gt
  where
    (lt,found,gt) = splitMember' cmp x t
...
deleteMin (Set cmp s) = Set cmp $ deleteMin' s
...
deleteMax (Set cmp s) = Set cmp $ deleteMax' s
...
unions :: Order a -> [Set a] -> Set a
unions cmp ts
  = foldlStrict union (empty cmp) ts
...
union (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ union' cmp1 l r
union' cmp Tip t2  = t2
union' cmp t1 Tip  = t1
union' cmp t1 t2 = hedgeUnion' cmp (const LT) (const GT) t1 t2

hedgeUnion :: (a -> Ordering) -> (a -> Ordering) -> Set a -> Set a -> Set a
hedgeUnion cmplo cmphi (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ hedgeUnion' cmp1 cmplo cmphi l r
hedgeUnion' cmp _     _     t1 Tip
  = t1
hedgeUnion' cmp cmplo cmphi Tip (Bin _ x l r)
  = join' x (filterGt' cmplo l) (filterLt' cmphi r)
hedgeUnion' cmp cmplo cmphi (Bin _ x l r) t2
  = join' x (hedgeUnion' cmp cmplo cmpx l (trim' cmplo cmpx t2)) 
           (hedgeUnion' cmp cmpx cmphi r (trim' cmpx cmphi t2))
  where
    cmpx y  = cmp x y
...
difference (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ difference' cmp1 l r
difference' cmp Tip _   = Tip
difference' cmp t1 Tip  = t1
difference' cmp t1 t2   = hedgeDiff' cmp (const LT) (const GT) t1 t2

hedgeDiff :: (a -> Ordering) -> (a -> Ordering) -> Set a -> Set a -> Set a
hedgeDiff cmplo cmphi (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ hedgeDiff' cmp1 cmplo cmphi l r
hedgeDiff' cmp _ _ Tip _
  = Tip
hedgeDiff' cmp cmplo cmphi (Bin _ x l r) Tip 
  = join' x (filterGt' cmplo l) (filterLt' cmphi r)
hedgeDiff' cmp cmplo cmphi t (Bin _ x l r) 
  = merge' (hedgeDiff' cmp cmplo cmpx (trim' cmplo cmpx t) l) 
          (hedgeDiff' cmp cmpx cmphi (trim' cmpx cmphi t) r)
  where
    cmpx y = cmp x y -- compare x y
...
intersection (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ intersection' cmp1 l r
intersection' cmp Tip _ = Tip
intersection' cmp _ Tip = Tip
intersection' cmp t1@(Bin s1 x1 l1 r1) t2@(Bin s2 x2 l2 r2) =
   if s1 >= s2 then
      let (lt,found,gt) = splitLookup' cmp x2 t1
          tl            = intersection' cmp lt l2
          tr            = intersection' cmp gt r2
      in case found of
      Just x -> join' x tl tr
      Nothing -> merge' tl tr
   else let (lt,found,gt) = splitMember' cmp x1 t2
            tl            = intersection' cmp l1 lt
            tr            = intersection' cmp r1 gt
...
filter p (Set cmp s) = Set cmp $ filter' p s
filter' _ Tip = Tip
...
partition p (Set cmp s) = case partition' p s of (l,r) -> (Set cmp l, Set cmp r)
...
mapMonotonic :: Order b -> (a->b) -> Set a -> Set b
mapMonotonic cmpB f (Set cmp s) = Set cmpB $ mapMonotonic' f s
...
foldr f z (Set cmp s) = foldr' f z s
foldr' _ z Tip           = z
...
-- | /O(n*log n)/. Create a set from a list of elements, with a given order.
fromList :: Order a -> [a] -> Set a 
fromList cmp xs = Set cmp $ fromList' cmp xs 
fromList' cmp xs 
...
-- /The precondition (input list is ascending wrt the order) is not checked./
fromAscList :: Order a -> [a] -> Set a 
fromAscList cmp xs = Set cmp $ fromAscList' cmp xs 
fromAscList' cmp xs
...
fromDistinctAscList :: Order a -> [a] -> Set a 
fromDistinctAscList cmp xs = Set cmp $ fromDistinctAscList' xs
...
trim cmplo cmphi (Set cmp s) = Set cmp $ trim' cmplo cmphi s
trim' _     _     Tip = Tip
...
filterGt cmp1 (Set cmp s) = Set cmp (filterGt' cmp1 s)
filterGt' _ Tip = Tip
...
filterLt cmp1 (Set cmp s) = Set cmp (filterLt' cmp1 s)
filterLt' _ Tip = Tip
...
split x (Set cmp s) = case split' cmp x s of 
	(l,r) -> (Set cmp l, Set cmp r)
split' cmp _ Tip = (Tip,Tip)
split' cmp x (Bin _ y l r)
  = case cmp x y of
      LT -> let (lt,gt) = split' cmp x l in (lt,join' y gt r)
      GT -> let (lt,gt) = split' cmp x r in (join' y l lt,gt)
      EQ -> (l,r)

-- | /O(log n)/. Performs a 'split' but also returns whether the pivot
-- element was found in the original set.
splitMember :: {- Ord a => -} a -> Set a -> (Set a,Bool,Set a)
splitMember x (Set cmp s) = case splitMember' cmp x s of (l,f,r) -> (Set cmp l, f, Set cmp r)
splitMember' cmp x t = let (l,m,r) = splitLookup' cmp x t in
...
splitLookup x (Set cmp s) = case splitLookup' cmp x s of
	(l,ma,r) -> (Set cmp l, ma, Set cmp r)
splitLookup' cmp _ Tip = (Tip,Nothing,Tip)
splitLookup' cmp x (Bin _ y l r)
   = case cmp x y of
       LT -> let (lt,found,gt) = splitLookup' cmp x l in (lt,found,join' y gt r)
       GT -> let (lt,found,gt) = splitLookup' cmp x r in (join' y l lt,found,gt)
       EQ -> (l,Just y,r)
...
join x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ join' x l r
...
insertMax x (Set cmp s) = Set cmp $ insertMax' x s
insertMin x (Set cmp s) = Set cmp $ insertMin' x s
insertMax' x t
  = case t of
      Tip -> singleton' x
      Bin _ y l r
...
merge (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ merge' l r
merge' Tip r   = r
merge' l Tip   = l
...
glue (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ glue' l r
glue' Tip r = r
glue' l Tip = l
glue' l r   
...
deleteFindMin (Set cmp s) = case deleteFindMin' s of
	(a,s') -> (a,Set cmp s')
deleteFindMin' t 
  = case t of
      Bin _ x Tip r -> (x,r)
...
deleteFindMax (Set cmp s) = case deleteFindMax' s of
	(a,s') -> (a,Set cmp s')
deleteFindMax' t
  = case t of
      Bin _ x l Tip -> (x,l)
...
minView (Set cmp s) = case (minView' s) of
	Nothing -> Nothing
	Just (x,s') -> Just (x, Set cmp s')
minView' Tip = Nothing
...
maxView (Set cmp s) = case (maxView' s) of
	Nothing -> Nothing
	Just (x,s') -> Just (x, Set cmp s')
maxView' Tip = Nothing
...
balance x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ balance' x l r
balance' x l r
...
rotateL x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ rotateL' x l r
...
rotateR x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ rotateR' x l r
...
singleL x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ singleL' x l r
singleR x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ singleR' x l r
...
doubleL x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ doubleL' x l r
doubleR x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ doubleR' x l r
...
bin x (Set cmp1 l) (Set cmp2 r) = Set cmp1 $ bin' x l r
bin' x l r
...
showsTree wide lbars rbars (Set cmp t) = showsTree' wide lbars rbars t
...
showsTreeHang b ss (Set cmp t) = showsTreeHang' b ss t
...
ordered (Set cmp t) = ordered' cmp t
ordered' cmp t
  ...
  where
    bounded lo hi t'
      = case t' of
          Tip         -> True
          Bin _ x l r -> (lo x) && (hi x) && bounded lo (<? x) l && bounded (>? x) hi r where (<?) = lt cmp ; (>?) = gt cmp

balanced :: Set a -> Bool
balanced (Set cmp t) = balanced' t
balanced' t
  = case t of
      Tip         -> True
...
validsize (Set cmp t) = validsize' t
validsize' t
